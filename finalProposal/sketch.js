...
function setup(){
  ...
  eSlider.position(width/2+300,height/2+110);
  eSlider.size(70,3);
  //hSlider
  hrSlider = createSlider(0,255,100);
  hrSlider.position(width/2+300,height/2+150);
  hrSlider.size(70,3);
  //pSlider
  pSlider = createSlider(0,255,100);
  pSlider.position(width/2+300,height/2 +190);
  pSlider.size(70,3);
  //input text
  nameInput = createInput("","text");
  nameInput.position(width/2+270,height/2-75);
  ...
  farmInput.position(width/2+270,height/2-35);
  ...
  favouriteInput.position(width/2+270,height/2+5);
...
function farm(){
  ...
  inventoryDisplay.checkTool();
  inventoryDisplay.checkNB();
  if(theWorld.offsetY== -1035 && theWorld.offsetX < -760 && theWorld.offsetX > -840 && pCurrent == pFront0){
    stage = 5;
    theRanch.offsetX = 0;
    theRanch.offsetY = 0;
  }
  //console.log(theWorld.offsetY);
  if(theWorld.offsetY > 905 && theWorld.offsetY<950 && theWorld.offsetX> -790 && pCurrent == pRight0){
    stage = 6;
    thePlayerInP.x = 200;
    thePlayerInP.y = 450;
...
function animalShop(){
  ...
  if(theRanch.offsetX > 0 && theRanch.offsetX < 40 && theRanch.offsetY < -20){
    stage = 4;
    theWorld.offsetX = -835;
    theWorld.offsetY = -1020;
    //heWorld.offsetX -835
  }
...
function shop(){
  ...
  if(thePlayerInP.y>450 && thePlayerInP.x>180 && thePlayerInP.x<210){
    stage = 4;
    theWorld.offsetX = -750;
    theWorld.offsetY = 920;
  }
  if(purchaseStage == 1){
    imageMode(CENTER);
    image(shopList,width/2,height/2-70,shopList.width/4,shopList.height/4);
    //choose parsnip
    if(mouseY>94 && mouseY < 140 && mouseX>225 && mouseX<730){
      hover_parsnip = true;
      noStroke();
      fill(38,100,29,50);
      rect(220,94,520,50);
    ...
    //choose bean
    if(mouseX>225 && mouseX<730 && mouseY>140 && mouseY<190){
      hover_bean = true;
      noStroke();
      fill(38,100,29,50);
      rect(220,145,520,50);
    ...
    //choose cauliflower
    if(mouseX>225 && mouseX<730 && mouseY>190 && mouseY<240){
      hover_cauli = true;
      noStroke();
      fill(38,100,29,50);
      rect(220,200,520,50);
    ...
    //choose potato
    if(mouseX>225 && mouseX<730 && mouseY>240 && mouseY<290){
      hover_potato = true;
      noStroke();
      fill(38,100,29,50);
      rect(220,255,520,50);
    ...
    //closeButton
    if(mouseX>width/2+250 && mouseX<width/2+310 && mouseY<height/3-40 && mouseY>height/3-100){
      tint(240,20,100);
      hover_finish = true;
...
class Inventories{
  constructor(){
    if(stage == 4){
      this.x = width/2;
      this.y = height-50;
    }else if (stage == 5 || stage == 6 || stage == 3){
      this.x = width/2;
      this.y = 50;
    }
...
function mouseClicked(){
  ...
  if(hover_potato == true){
    if(potatoNB<1){
      invent.push(potatoSeed);
    }
    potatoNB++;
  }
  if(hover_parsnip == true){
    if(parsnipNB < 1){
      invent.push(parsnipSeed);
    }
    parsnipNB++;
